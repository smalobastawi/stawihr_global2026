                ...
                <div class="col-md-offset-2 col-md-7 ">
                    <div class="panel panel-default">
                        <div class="panel-heading">{{ $value->applicant_name }} <span class="applicationDate">&nbsp;
                                Called for interview </span>{{ $job->post }} <span
                                class="applicationDate">&nbsp;Position </span></div>
                        <div class="panel-wrapper collapse in">
                            <div class="panel-body">
                                ...
                                <a class="downloadResume" href="{{ route('view.CV', $value->job_applicant_id) }}" download="" target="_blank"i
                                        class="fa fa-download"></i> Download Resume</a>
                            </div>
                            <div class="panel-footer">
                                <p>
                                    <b>Date :</b>
                                    @if (isset($value->interviewInfo->interview_date))
                                        {{ date(' d M Y ', strtotime($value->interviewInfo->interview_date)) }}
                                    @endif&nbsp;
                                    <b>Time :</b>
                                    @if (isset($value->interviewInfo->interview_time))
                                        {{ date('g:i A', strtotime($value->interviewInfo->interview_time)) }}
                                    @endif
                                    <b> 
									<br>	
									<a href="{{ route('applicant.hire', [
                                        'id' => $value->job_applicant_id,
                                    ]) }}"
                                            onclick="return confirm('you are want to HIRE this applicant. Are you sure?')">
                                            <button type="submit" class="btn btn-info"><i class="fa fa-check"></i>
                                                Hire</button></a>
                                        <a href="{{ route('applicant.reject', $value->job_applicant_id) }}"
                                            onclick="return confirm('you are want to reject this applicant. Are you sure?')">
                                            <button type="submit" class="btn btn-danger"><i class="fa fa-eraser"></i>
                                                Reject Application</button></a>
