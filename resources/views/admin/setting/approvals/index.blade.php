								...
								<tbody>
									...
									@foreach($results AS $value)
										<tr class="{!! $value->id !!}">
											...
											<td>{!! $value->approvers_list!!}</td>
											<td>{!! $value->approver_numbers !!}</td>
											<td style="width: 100px;">
												<a href="{!! route('approvalSettings.edit',$value->id) !!}"  class="btn btn-success btn-xs btnColor">
													<i class="fa fa-pencil-square-o" aria-hidden="true"></i>
												</a>
												<a href="{!!route('approvalSettings.delete',$value->id )!!}" data-token="{!! csrf_token() !!}" data-id="{!! $value->id!!}" class="delete btn btn-danger btn-xs deleteBtn btnColor"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
											</td>
										</tr>
									@endforeach
								</tbody>
