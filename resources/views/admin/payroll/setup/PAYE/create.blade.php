    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="panel panel-info">
                <div class="panel-heading"><i class="mdi mdi-table fa-fw"></i> Add Tax Band</div>
                <div class="panel-wrapper collapse in" aria-expanded="true">
                    <div class="panel-body">
                        @if($errors->any())
                            <div class="alert alert-danger alert-dismissable">
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                                <i class="glyphicon glyphicon-remove"></i>&nbsp;<strong>Please fix the following errors:</strong>
                                <ul>
                        ...
                        <form method="POST" action="{{ route('tax-bands.store') }}">
                            @csrf
                            
                            <div class="form-group row">
                                <label class="col-md-2 control-label">Country<span class="validateRq">*</span></label>
                                <div class="col-md-4">
                                    <select class="form-control select2 @error('country_id') is-invalid @enderror" name="country_id" id="country_id" required>
                                        <option value="">Select Country</option>
                                        @foreach(Nationality::toArray() as $key => $country)
                                ...
                                <div class="col-md-4">
                                    <button type="button" id="load-kenya-preset" class="btn btn-success" style="display: none;">
                                        <i class="fa fa-magic"></i> Load Kenya (KRA) Preset Rates
                                    </button>
                                    <span id="preset-info" class="text-muted" style="display: none; margin-left: 10px;">
                                        <i class="fa fa-info-circle"></i> Rates from July 1, 2023
                                    </span>
                                </div>
                            </div>
                            <div class="form-group row">
                                <div class="col-md-offset-2 col-md-8">
                                    <div class="alert alert-info" style="padding: 8px 15px; margin-bottom: 0;">
                                        <i class="fa fa-lightbulb-o"></i> <strong>Tip:</strong> Enter <strong>monthly</strong> amounts only. Annual amounts will be auto-calculated (×12).
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered" id="taxTable">
                                    ...
                                    <tbody>
                                        @php
                                            $oldBands = old('bands', [['monthly_lower_bound' => '', 'monthly_upper_bound' => '', 'annual_lower_bound' => '', 'annual_upper_bound' => '', 'tax_rate' => '', 'band_order' => 1]]);
                                        @endphp
                            ...
                            <div class="form-actions">
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <a href="{{ route('tax-bands.index') }}" class="btn btn-default btn_style">
...
<script>
    $(document).ready(function() {
        const KENYA_ID = '88'; // Kenya's ID from Nationality enum

        // Kenya KRA PAYE Tax Bands (Effective July 1, 2023)
        const kenyaKraBands = [
            { monthly_lower: 0, monthly_upper: 24000, annual_lower: 0, annual_upper: 288000, tax_rate: 10 },
            { monthly_lower: 24001, monthly_upper: 32333, annual_lower: 288001, annual_upper: 388000, tax_rate: 25 },
            { monthly_lower: 32334, monthly_upper: 500000, annual_lower: 388001, annual_upper: 5600000, tax_rate: 30 },
            { monthly_lower: 500001, monthly_upper: 800000, annual_lower: 5600001, annual_upper: 9600000, tax_rate: 32.5 },
            { monthly_lower: 800001, monthly_upper: null, annual_lower: 9600001, annual_upper: null, tax_rate: 35 }
        ];

        // Show/hide Kenya preset button based on country selection
        $('#country_id').change(function() {
            const selectedCountry = $(this).val();
            if (selectedCountry === KENYA_ID) {
                $('#load-kenya-preset').show();
                $('#preset-info').show();
            } else {
                $('#load-kenya-preset').hide();
                $('#preset-info').hide();
            }
        });

        // Load Kenya KRA preset rates
        $('#load-kenya-preset').click(function() {
            if (confirm('This will replace all current tax bands with Kenya KRA PAYE rates (effective July 1, 2023). Continue?')) {
                loadKenyaPreset();
            }
        });

        function loadKenyaPreset() {
            // Clear existing rows
            $('#taxTable tbody').empty();

            // Add Kenya preset bands
            kenyaKraBands.forEach((band, index) => {
                const maxDisplay = band.monthly_upper ? band.monthly_upper : '';
                const annualMaxDisplay = band.annual_upper ? band.annual_upper : '';
                const upperPlaceholder = band.monthly_upper ? band.monthly_upper : '∞';
                ...
                const newRow = `
                    <tr class="band-row">
                        ...
                        <td>
                            <input type="number" name="bands[${index}][band_order]"
                                   class="form-control band-order"
                                   value="${index + 1}" min="1" required readonly>
                        </td>
                        <td>
                            <div class="form-group" style="margin-bottom: 0;">
                                <div class="row" style="margin: 0 -5px;">
            ...
            toastr.success('Kenya KRA PAYE rates loaded successfully!');
        }
