...
{{--				<a href="{{route('calculatePaye')}}" class="btn btn-primary pull-right">Calculate Paye</a>--}}
				<a href="{{ route('generateSalarySheet.create') }}"  class="btn btn-success pull-right m-l-20  waves-effect waves-light"> <i class="fa fa-plus-circle" aria-hidden="true"></i> @lang('salary_sheet.generate_salary_sheet')</a>
				<a href="{{ route('downloadFullPayroll') }}"  class="btn btn-success pull-right m-l-20  waves-effect waves-light"> <i class="fa fa-plus-circle" aria-hidden="true"></i>Generate Payslips PDF</a>
				<a href="{{ route('generatePayrollExcel') }}"  class="btn btn-success pull-right m-l-20  waves-effect waves-light"> <i class="fa fa-plus-circle" aria-hidden="true"></i>Generate Excel</a>
			</div>
		</div>
		<div class="row">
			<div class="col-sm-12">
...
@foreach(array(''=>'---- '. __('common.please_select') .' ----','0' => __('salary_sheet.unpaid'), '1' => __('salary_sheet.paid')) as $__key => $__value)
<option value="{{ $__key }}" {{ (string)'' == (string)$__key ? 'selected' : '' }}>{{ $__value }}</option>
@endforeach
</select>
									</div>
								</div>
