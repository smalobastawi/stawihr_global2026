                    ...
                    <div class="panel-wrapper collapse in" aria-expanded="true">
                        <div class="panel-body">
                            ...
                            <div class="filter-section">
                                <form id="deductionsReportFilter" method="GET"
                                    action="{{ route('payroll.reports.deductions') }}">
                                    <div class="row filter-row">
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <select name="deduction_id" id="deduction_id_filter"
                                                    class="form-control select2">
                                                    <option value="">All Deduction Types</option>
                                                    @foreach($deductionTypes as $deductionType)
                                                        <option value="{{ $deductionType->deduction_id }}" {{ request('deduction_id') == $deductionType->deduction_id ? 'selected' : '' }}>
                                                            {{ $deductionType->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                    ...
                                    <div class="row filter-row">
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group" style="margin-top: 25px;">
                                                <a href="{{ route('payroll.reports.deductions') }}" class="btn btn-warning btn-block">
                            ...
                            <div class="table-responsive">
                                <table id="deductionsReportTable" class="table table-bordered" style="width:100%">
                                    <thead>
                                        <tr>
                                            <th>Employee Name</th>
                                            <th>Department</th>
...
@section('page_scripts')
    <script>
        $(document).ready(function() {
            ...
            if ($('#deductionsReportTable tbody tr').length > 1 || ($('#deductionsReportTable tbody tr').length === 1 && $('#deductionsReportTable tbody tr td').length > 1)) {
                $('#deductionsReportTable').DataTable({
                    "paging": true,
                    "ordering": true,
                    "info": true,
                    "searching": true
            ...
            $('#downloadExcelButton').on('click', function(e) {
                e.preventDefault();
                var form = $('#deductionsReportFilter');
                var originalAction = form.attr('action');
                var originalMethod = form.attr('method');
                form.attr('action', '{{ route("payroll.reports.deductions.export") }}');
