			...
			<div class="panel panel-info">
				...
				<div class="panel-wrapper collapse in" aria-expanded="true">
					<div class="panel-body">
						<div class="row">
							<div id="searchBox">
								<form action="{{ route('myLeaveReport.myLeaveReport.download') }}" method="POST" id="leaveReport">
@csrf
								...
								<div class="col-md-3">
									...
									<div class="input-group">
										...
										<input type="text" class="form-control dateField required" readonly placeholder="@lang('common.from_date')"  name="from_date" value="@if(isset($from_date)) {{$from_date}}@else {{ dateConvertDBtoForm(date($fiscal_start_date)) }} @endif">
									</div>
								</div>
								...
								<div class="col-md-3">
									...
									<div class="input-group">
										<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
										<input type="text" class="form-control dateField required" readonly placeholder="@lang('common.to_date')"  name="to_date" value="@if(isset($to_date)) {{$to_date}}@else {{ dateConvertDBtoForm(date($fiscal_end_date)) }} @endif">
									</div>
								</div>
								<div class="col-md-2">
						...
						@if(count($results) > 0)
							<h4 class="text-right">
								@if(isset($from_date))
									<a class="btn btn-success" style="color: #fff" href="{{ URL('downloadMyLeaveReport/?employee_id='.session('logged_session_data.employee_id').'&from_date='.$from_date.'&to_date='.$to_date)}}"><i class="fa fa-download fa-lg" aria-hidden="true"></i> @lang('common.download') PDF</a>
								@else
									<a class="btn btn-success" style="color: #fff" href="{{ URL('downloadMyLeaveReport/?employee_id='.session('logged_session_data.employee_id').'&from_date='.dateConvertDBtoForm(date($fiscal_end_date)) .'&to_date='.dateConvertDBtoForm(date($fiscal_end_date)) )}}"><i class="fa fa-download fa-lg" aria-hidden="true"></i>  @lang('common.download') PDF</a>
								@endif
							</h4>
						@endif
                        @if(!empty($results))
                                    ...
                                    <tbody>
                                        @if(count($results) > 0)
                                            ...
                                            <tr>
                                                ...
                                                <td>@if(isset($value->approveBy->first_name)) {{$value->approveBy->first_name}} {{$value->approveBy->last_name}} @endif</td>
                                                <td>{{dateConvertDBtoForm($value->ceo_approval_date)}}</td>
