...
class GenerateBonusController extends Controller
{
    ...
    public function index() {
        $results = EmployeeBonus::with('employee.employeePayroll','employee.hourlySalaries','festivalName')->orderBy('employee_bonus_id','DESC')->get();
    ...
    public function filter(Request $request) {
        ...
        $employee         = Employee::select(DB::raw('CONCAT(COALESCE(employee.first_name,\'\'),\' \',COALESCE(employee.last_name,\'\')) AS fullName'),'employee.date_of_birth',
                                'employee.gender','department.department_name','employee.employee_id',
                                'employee_payrolls.basic_salary','employee_payrolls.gross_salary','hourly_salaries.hourly_grade','hourly_salaries.hourly_rate','designation.designation_name')
                            ...
                            ->leftJoin('employee_payrolls', 'employee_payrolls.employee_id', '=', 'employee.employee_id')
    ...
    public function store(Request $request) {
        ...
        if($bug == 0){
            ...
            return redirect('generateBonus')->with('error', 'An error occured, please try again. If the problem persists, contact Support for assistance. ');
        }
    }
}
